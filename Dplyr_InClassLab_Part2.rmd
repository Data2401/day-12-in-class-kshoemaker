...
my_flights %>% mutate(gain = dep_delay - arr_delay, speed = distance / air_time * 60)

my_flights <- my_flights %>% mutate(gain = dep_delay - arr_delay, 
...
```{r}
my_flights %>% arrange(desc(gain))
my_flights %>% arrange(-gain) %>% head(6)## same thing
my_flights %>% slice_max(gain, n = 6)
```

4. If you haven't already, do the last 2 steps in a single statement using the pipe operator. 

```{r}
my_flights_piped <- my_flights %>% 
...
mean(my_flights$speed, na.rm = T)
mean(my_flights$arr_delay, na.rm = T)
```
...
```{r}
houston_flights <- my_flights %>% select(origin, dest, gain) %>% filter(dest == "HOU")
houston_flights <- my_flights %>% select(origin, dest, gain) %>% filter(dest %in% c("HOU", "IAH"))
```
...
mean(houston_flights$gain, na.rm = T)
...
```{r}
# Doesn't work, b/c of the NA
my_flights %>% 
  filter(origin == "JFK", dest == "SEA") %>% 
  summarise(across(air_time, list(mean = mean, max = max, min = min)))
...
my_flights %>% filter(dest %in% c("HOU", "IAH")) %>% 
  group_by(origin) %>% 
  summarise(mean_delay = mean(arr_delay, na.rm = T)) %>% arrange(mean_delay)
...
```{r}
starwars %>% group_by(homeworld) %>% summarise(n_characters = n())
...
starwars %>% group_by(eye_color) %>% summarise(mean_height = mean(height))
